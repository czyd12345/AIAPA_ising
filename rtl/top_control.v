...
// Description : Top-level annealing control FSM.
//
//               Broadcasts instructions to the SPU, tracks the number of
//               accepted spin flips per sweep (fsn), schedules the per-sweep
//               temperature / momentum parameter updates, and streams the
//               final spin configuration out over UART.
//
//               state 3'd1 : IDLE / config latch (also the UART drain state)
//               state 3'd2 : parameter broadcast
//                            T *= alpha, pk -= 1/2000, ck += 1/1000
//               state 3'd3 : momentum pass   (opcode 000001, "f + M*sigma")
//               state 3'd4 : field pass      (opcode 000010, "f + J*sigma")
//               state 3'd5 : parallel mode - fully commented out, dead code
//               state 3'd6 : parallel mode - fully commented out, dead code
//
// Parameters  : E = spin-block divisor, L = sparsity threshold for the
//               parallel-mode switch, P = readout group width.
//               NOTE: module parameter P (readout group width) is unrelated to
//               the input port in_p, which carries pk_start.
//
// See docs/architecture.md for the 32-bit packet formats.
//////////////////////////////////////////////////////////////////////////////////
module top_control #(
    parameter E=63,
    parameter L=6,
    ...
    input rst,
    input ap_start,
    input [31:0] datain_pkt,// (reserved: extra inputs)
    input [31:0] in_n,
    input [31:0] in_k,
    input [15:0] in_p,
    input [15:0] in_c,
    ...
    output reg mem_r,
    output [31:0] mem_addr, // off-chip address
    output rd_en_out,
  ...
  //  parameter E = 63,L=6,P=64;
    ...
    wire [15:0] result;// cooling coefficient
    reg [2:0] state;
    reg [31:0] N,K,F,Fl,Fr,Fl_last,Fr_last,F_new; // config inputs / spin counts
    reg [15:0] addr;  // on-chip address
    ...
    initial begin
        // Initial spin configuration for the two replicas. Paths are relative
        // to the simulation working directory (the repository root).
        // Generated by sw/gen_config.py.
        $readmemb("data/spinsl.txt",spinstate_l);
        $readmemb("data/spinsr.txt",spinstate_r);
    ...
    begin
        ...
        ctrl_flipspin  <= 1'b0;
        ctrl_up        <= 1'b0;        
        state <= state; // default: hold state
        m     <= m;
        i     <= i; k <= k; c <= c; f <= f; 
        ...
        case(state)
            ...
            3'd2: begin
        // send the three parameters one at a time
        case (m)
          0: begin
            para   <= t;
            coeff  <= alpha;
        ...
        endcase
            ...
            begin  // annealing
                ...
                begin
                    data_type<=0;
                    if(F==0) 
                    begin // make sure all data has been read into the memory
                        i<=1;
                        m<=0;
                        if(ctrl_lr) 
                        begin
...
/*            3'd5: begin // parallel mode
    ...
    MuxKeyWithDefault #(3,3,32) mux0(data_pkt,data_type,32'b0,{
        ...
        3'd4,{3'b100,5'b0,num[6:0],spinindex_l[f],ctrl_lr?spinstate_l[f]:spinstate_r[f]} // routed to the matching position by the zero-select block; dropped otherwise
    });//32=7+18+3+4=3+16+13
...
float_mul i0(para,coeff,result1);
